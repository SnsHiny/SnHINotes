...
- 修改克隆虚拟机参数
	...
	- mac地址（网络适配器-高级）
	- ip地址（vim /etc/sysconfig/network-scripts/ifcfg-ens33；systemctl restart network）
	- 主机名（vim /etc/hostname；i进入编辑模式；reboot重启）
	- UUID

	[^mac地址修改后需重新配置]: https://www.cnblogs.com/study10000/p/13153051.html
